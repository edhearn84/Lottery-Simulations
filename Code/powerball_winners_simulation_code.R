...
n_sims = 1e4
lottery_sims = lapply( rep(90,n_sims) , function(x) sim_lottos(x,ball_count = 59) )
...
real_sims %>%
  ...
  labs( title = "59-35 Powerball (01/2009 - 10/2015) Wins" ,
        y = "Ball number (+/-2 SD bands)" ,
        x = "Average number of occurrences in 90-jp simulations\n(Red dots are actual occurrences)" ) +
...
# 0 occurrences added in (ball numbers with no jackpot occurrences)
zeros_data_frame = tibble( number = c(15,35,39,41,46,47,49,50,63,64,69) ,
                           n = rep(0 , 11) )
...
# Simulate lottery draws for specific period of semi-weekly draws 
# (23 = number of won jackpots since change)
n_sims = 1e4
lottery_sims = lapply( rep(23,n_sims) , function(x) sim_lottos(x,ball_count = 69) )
...
real_sims %>%
  ...
  theme( axis.ticks.y = element_blank() ,
         axis.text.y = element_blank() ) +
  labs( title = "69-26 Powerball (after 10/2015) Wins" ,
        y = "Ball number (+2 SD band)" ,
        x = "Average number of occurrences in 23-jp simulations\n(Red dots are actual occurrences)" ) +
  ggthemes::theme_tufte() +
  theme(text = element_text(size = 15))
