...
zeros_data_frame = tibble( number = 51 ,
                           n = 0 )
...
# 49 = number of won jackpots
n_sims = 1e4
lottery_sims = lapply( rep(49,n_sims) , function(x) sim_lottos(x,ball_count = 56) )
...
real_sims %>%
  ...
  labs( title = "56-46 Mega Millions Wins" ,
...
# 0 occurences added in (ball numbers with no jackpot occurences)
zeros_data_frame = tibble( number = c(4,12,16,43,47,52,68,71,72) ,
                           n = rep(0,9) )
...
# Simulate lottery draws for specific period of semi-weekly draws 
# 32 = number of won jackpots
n_sims = 1e4
lottery_sims = lapply( rep(32,n_sims) , function(x) sim_lottos(x,ball_count = 75) )
...
real_sims %>%
  ...
  theme( axis.ticks.y = element_blank() ,
         axis.text.y = element_blank() ) +
  labs( title = "75-15 Mega Millions Wins" ,
        y = "Ball number (+2/-1 SD bands)" ,
...
# 0 occurences added in (ball numbers with no jackpot occurences)
zeros_data_frame = tibble( number = c(3,6:9,13:17,20:27,32:33,35:38,40:43,45,47:56,58,60:61,63:64,66:69) ,
                           n = rep(0,48) )
...
# Simulate lottery draws for specific period of semi-weekly draws 
# 6 = number of won jackpots
n_sims = 1e4
lottery_sims = lapply( rep(6,n_sims) , function(x) sim_lottos(x,ball_count = 70) )
...
real_sims %>%
  ggplot( aes(y = number , x = n_avg) ) +
  geom_text( aes( label = number ) , size = 3 ) +
  geom_point( aes(x = 0 , y = number) , pch = 124) +
  geom_point( aes(x = n_avg + 2*n_sd , y = number) , pch = 124) +
  geom_point( aes(x = n , y = number) , col = "red" ) +
  theme( axis.ticks.y = element_blank() ,
         axis.text.y = element_blank() ) +
  labs( title = "70-25 Mega Millions Wins" ,
