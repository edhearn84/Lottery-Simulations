...
n_sims = 1e4
lottery_sims = lapply( rep(276,n_sims) , function(x) sim_lottos(x,ball_count = 39) )
...
real_sims %>%
  ...
  labs( title = "1-39 Powerball (before 01/2012) Rolls" ,
        y = "Ball number (+2/-2 SD bands)" ,
        x = "Average number of occurences in 276-jp simulations\n(Red dots are actual occurrences)" ) +
...
# Simulate lottery draws for specific period of semi-weekly draws
n_sims = 1e4
lottery_sims = lapply( rep(339,n_sims) , function(x) sim_lottos(x,ball_count = 35) )
...
real_sims %>%
  ...
  theme( axis.ticks.y = element_blank() ,
         axis.text.y = element_blank() ) +
  xlim(min = 0 , max = 20) +
  labs( title = "1-35 Powerball (01/2012 - 10/2015) Rolls" ,
        y = "Ball number (+2/-2 SD bands)" ,
...
# Simulate lottery draws for specific period of semi-weekly draws
n_sims = 1e4
lottery_sims = lapply( rep(322,n_sims) , function(x) sim_lottos(x,ball_count = 26) )
...
real_sims %>%
  ...
  theme( axis.ticks.y = element_blank() ,
         axis.text.y = element_blank() ) +
  labs( title = "1-26 Powerball (post-10/2015) Rolls" ,
        y = "Ball number (+2/-2 SD bands)" ,
        x = "Average number of occurences in 322-jp simulations\n(Red dots are actual occurrences)" ) +
  ggthemes::theme_tufte()
